...
opcion_enlaces_simbolicos_var = StringVar()
...
opcion_enlaces_simbolicos_inicial_str = None
...
idioma_seleccionado_inicial_str = None
color_negro = '#000'
color_rojo = '#f00'
idioma_seleccionado = ''
idioma_defecto = 'en'
...
def crea_menus():
    ...
    menubar = Menu(root)
    menu_opciones = Menu(menubar, tearoff=0)  # tearoff = 0 no deja que una ventana sea flotante
...
left_buttons_frame.place(x=28, y=398, height=100, width=430)
...
if os.path.isfile('urracabt.conf') == True:
    ...
    opcion_enlaces_simbolicos_var.set(opciones_dict.get('tratamiento_archivos_enlazados'))
    opcion_enlaces_simbolicos_inicial_str = opcion_enlaces_simbolicos_var.get()
    idioma_seleccionado = items_fichero_configuracion[5]
    idioma_seleccionado_inicial_str = idioma_seleccionado
...
def cuenta_atras(contador):
    ...
    if contador < 4:
        # Llama la cuenta atrás después de 1000 milisegundos.
        root.after(1000, cuenta_atras, contador + 1)
        
        # Se desea que la cadena de texto permanezca en pantalla si
...
def procesa_datos_origen(dry=False):
    ...
    texto_etiqueta_mensaje_de_estado.set('')
    if len(directorios_seleccionados_lstbox.curselection()) != 0:
        items_seleccionados = tuple()
        for i in range(len(directorios_seleccionados_lstbox.curselection())):
            indice = directorios_seleccionados_lstbox.curselection()[i]
            items_seleccionados += directorios_seleccionados_lstbox.get(indice, indice)
    ...
    if directorio_destino.find(':') == -1:
        if not os.path.exists(directorio_destino):
            ventana_msg(_('No existe directorio de destino'))
            return None
    ...
    for directorio_a_copiar in directorios_seleccionados_tupla:
        ...
        if opcion_delete_var.get() == 'True':
            indice_lista += 1
            tmp = comando_lst[:]  # En tmp instroduce la lista comando_lst
            tmp.insert(indice_lista, '--delete')
            comando_lst = tmp.copy()
        if opcion_comprimir_var.get() == 'True':
            tmp = comando_lst[:]  # En tmp_ introduce la lista comando_lst
            tmp[1] = tmp[1] + 'z'  # añade al literal "avP" el carácter 'z'
            comando_lst = tmp.copy()
        if opcion_enlaces_simbolicos_var.get() == 'mantener_enlaces_simbolicos':
            tmp = comando_lst[:]  # En tmp instroduce la lista comando_lst
            tmp.insert(3, '--links')
            comando_lst = tmp.copy()
        if opcion_enlaces_simbolicos_var.get() == 'copiar_archivos_enlazados':
            indice_lista += 1
            tmp = comando_lst[:]  # En tmp instroduce la lista comando_lst
        ...
        if dry:
            ...
            #print('Lista:',comandos_para_cron_lst)
            continue
...
def guarda_lista_items_seleccionados():
    ...
    global modificacion_de_items, opcion_comprimir_inicial_str, opcion_delete_inicial_str, \
           ...
           tipo_acceso_var_inicial_str, opcion_enlaces_simbolicos_inicial_str
    if not directorio_destino_txtbox.get('1.0', 'end').strip().find(':') == -1:
        if puerto_remoto_txtbox.get('1.0', 'end').strip() == '':
            messagebox.showinfo(title=nombre_aplicacion + ' ' + 'v.' + version,
                                message=_('Debe asignar un valor de puerto remoto para un destino remoto'))
    ...
    if not (modificacion_de_items or not opcion_delete_inicial_str == opcion_delete_var.get() or
            not opcion_comprimir_inicial_str == opcion_comprimir_var.get() or
            not opcion_enlaces_simbolicos_inicial_str == opcion_enlaces_simbolicos_var.get()):
    ...
    fichero_configuracion.write(_dict)
    fichero_configuracion.write('\n')
    if opcion_enlaces_simbolicos_var.get() == '':
        opcion_enlaces_simbolicos_var.set('copiar_archivos_enlazados')
    _dict = "{'tratamiento_archivos_enlazados':" + "'" + opcion_enlaces_simbolicos_var.get() + "'" + "}"
    fichero_configuracion.write(_dict)
    fichero_configuracion.write('\n')
...
def comprueba_cambios_y_salir():
    ...
    modificacion_radiobotones_ficheros_con_enlaces = False
    if not opcion_enlaces_simbolicos_inicial_str == opcion_enlaces_simbolicos_var.get():
        modificacion_radiobotones_ficheros_con_enlaces = True
    ...
    if modificacion_de_items or modificacion_checkboxes or modificacion_radiobotones_ficheros_con_enlaces:
        respuesta = askyesno(title=_('Confirmación'),
...
def callback(event):
    ...
    if len(directorios_seleccionados_lstbox.curselection()) != 0:
        if item_seleccionado == directorios_seleccionados_lstbox.curselection():
            directorios_seleccionados_lstbox.selection_clear(0, 'end')
...
opciones_ficheros_enlazados_lf = LabelFrame(root,  text=_('Opciones para ficheros enlazados'), width=350, height=85, font=(None, 12))
opciones_ficheros_enlazados_lf.place(x=35, y=235)
enlaces_simbolicos_mantener_enlace_radiobtn = Radiobutton(
    opciones_ficheros_enlazados_lf,
    text=_('Rsync enlaces simbólicos'),
    value='mantener_enlaces_simbolicos',
    variable=opcion_enlaces_simbolicos_var,
    font=("None", 12))
enlaces_simbolicos_mantener_enlace_radiobtn.place(x=6, y=4)
enlaces_simbolicos_copiar_fichero_radiobtn = Radiobutton(
    opciones_ficheros_enlazados_lf,
    text=_('Rsync archivos enlazados'),
    value='copiar_archivos_enlazados',
    variable=opcion_enlaces_simbolicos_var,
    font=("None", 12))
enlaces_simbolicos_copiar_fichero_radiobtn.place(x=6, y=30)
...
Label(root, text=_('Línea de mensajes:'), font=(None, 12)).place(x=37, y=495)
...
if idioma_seleccionado == 'es':
    ...
    x_pos = 155
mensaje_de_estado_lbl.place(x=x_pos, y=495)
directorios_seleccionados_lstbox = Listbox(dir_selected_lstbox_frame, width=40, height=8, font=(None, 12), selectmode='multiple')
...
option_delete_chkbtn.place(x=30, y=332)
option_comprimir_chkbtn = Checkbutton(root,
                                      ...
                                      onvalue='True',
                                      offvalue='False')
option_comprimir_chkbtn.place(x=30, y=357)
